...
IMAGE_NAME_FINAL = asterisk
IMAGE_TAG_FINAL = 13.13cert4
REPO_HOST = docker.io
...
ART_DIR = $(BASE_DIR)/artifacts/$(IMAGE_NAME_FINAL)
...
build:
	...
	docker build -t $(REPO_HOST)/$(REPO_NAME)/$(IMAGE_NAME_BUILDER) -f Dockerfile .
	...
	docker build -t $(REPO_HOST)/$(REPO_NAME)/$(IMAGE_NAME_FINAL):$(IMAGE_TAG_FINAL) -f asterisk13.13cert4-run.Dockerfile .
